...
async def download_with_json_index(
    ...
    logger.debug(f'{tag} 目录 {endpoint} 需要下载 {need_download_files} 个文件 (小: {len(small_files)}, 中: {len(medium_files)}, 大: {len(large_files)})')
    ...
    logger.info(f'{tag} 目录 {endpoint} 检查完成: 总数={total_files}, 已存在={exist_files}, 下载成功={success_count}, 失败={len(failed_files)}')
