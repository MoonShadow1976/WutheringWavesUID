...
class GroupRankRecord(SQLModel, table=True):
    ...
    async def save_record(
        cls, session: AsyncSession, user_id: str, waves_id: str, rank_type: str, challenge_id: int, data: dict
    ) -> "GroupRankRecord":
        """
        保存或更新排行记录
        """
        ...
        result = await session.execute(
            select(cls).where((cls.waves_id == waves_id) & (cls.rank_type == rank_type) & (cls.challenge_id == challenge_id))
        )
    ...
    async def get_group_records(
        ...
        statement = select(cls).where(*conditions).options(selectinload(cls.teams).selectinload(GroupRankTeam.roles))
        result = await session.execute(statement)
        return result.scalars().all()

    @classmethod
    @with_session
    async def clean_records(cls, session: AsyncSession, rank_type: str | None = None):
        """
        清理记录
        """
        if rank_type:
